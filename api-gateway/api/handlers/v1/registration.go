...
import (
	...
	token "projects/article/api-gateway/api/handlers/tokens"
	"projects/article/api-gateway/pkg/etc"
	l "projects/article/api-gateway/pkg/logger"
	pbu "projects/article/api-gateway/protos/template-service"
...
func (h *handlerV1) SignUp(c *gin.Context) {
	...
	err = h.writer.ProducerMessage("test-topic", byteData)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "error while producing to kafka",
		})
		h.log.Error("storing to the kafka error", l.Error(err))
		return
	}
	...
	auth := smtp.PlainAuth("", "keldimurodovsardor@gmail.com", "ghgybsvnstezkztx", "smtp.gmail.com")
